...
class TopTronicTextSensor : public text_sensor::TextSensor, public TopTronicBase {
 ...
 protected:
  std::map<uint8_t, std::string> to_text_;
  std::map<std::string, uint8_t> to_value_;
};
...
class TopTronicDevice {
 public:
  // Non-owning pointers: sensor/input lifetime is managed by the ESPHome component registry.
...
class TopTronic : public Component {
 ...
 protected:
  ...
  // Owned devices — unique_ptr ensures automatic cleanup on TopTronic destruction.
  std::map<uint32_t, std::unique_ptr<TopTronicDevice>> devices_;
  // msg_header -> (accumulated data, number of pending fragments)
