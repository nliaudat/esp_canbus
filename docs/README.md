...
| [`devices.md`](devices.md) | **Supported devices & bus addresses** — every device type the `toptronic:` component can talk to, its description, default CAN bus address (from the Hoval documentation), and how node addressing (`device_type | device_addr`) works. |
| [`toptronic_internals.md`](toptronic_internals.md) | **Component internals (recent major changes)** — multi-hub entity identity (auto device-type/address/bus prefixes, `name_prefix`, `/` sanitization, unique preset ids, duplicate-hub validation), the reassembly start-frame filter, and the offline candump replay tool. |
