...
- `USE_FEATURE_NAME` — feature enabled by a YAML option
- `DEBUG_FEATURE_NAME` — debug/diagnostic path
- `CONFIG_FEATURE_NAME` — build-configuration knobs

**Rules:**

- Every optional code path MUST be guarded — guard BOTH the declaration (header) and every call site (`.cpp`), so the feature compiles out of the binary when disabled.
- In `__init__.py`, define the macro ONLY when the corresponding YAML option is enabled:
...
- **NEVER commit `#if 0` / `#if 1` blocks** — that is dead code; delete it instead.
...
Default to `protected` (the ESPHome convention — derived entity classes share hub state through the protected base). Use `private` ONLY for safety-critical cases:

1. **Pointer lifetime safety** — members that must point into a validated, owned list.
2. **Invariant coupling** — two members that must stay synchronized (e.g. buffer size + pointer).
3. **Resource management** — cleanup that a derived class might skip.

```cpp
// Store the pointer FROM the validated list — never the raw input pointer
bool set_custom_mode(const char *mode) {
  const char *validated = this->find_in_list(mode);
  if (validated) {
    this->active_custom_mode_ = validated;  // Safe: points into modes_
    return true;
  }
  return false;
}
...
private:
  std::vector<const char *> modes_;            // string literals in flash
  const char *active_custom_mode_{nullptr};    // MUST point into modes_
```
...
### 9.1 Long-Running Device Rules

- **DO** allocate everything in `setup()`.
- **DO** use `std::array` or a fixed-size collection for fixed-size buffers (reassembly buffer, frame payloads).
- **DO NOT** `new`/`delete` repeatedly in `loop()` or the CAN callback.
- **DO NOT** use `std::string` for temporary values in the hot path (`parse_frame()`, `interpret_message()`, `send_can_frames()`).

### 9.2 RAII & Ownership

- Own buffers/objects with `std::unique_ptr` (`std::make_unique<...>`) or containers — never raw owning pointers. This matches the existing `std::unique_ptr<TopTronicDevice>` device map in the hub.
- No manual `free()`/`delete` of owned objects; destructors clean up on scope exit.

### 9.3 Thread/Task Safety

- The CAN receive path delivers frames via callback and can run outside the main `loop()` task. Shared mutable state between the callback and `loop()` MUST be guarded with `std::mutex` + `std::lock_guard` (or `std::atomic` for scalars).
- Never mutate a `std::vector`/`std::map`/`std::deque` from the callback while `loop()` reads it.
...
- Guarded multiplication — fail closed BEFORE the overflow (CVE-2026-23833 pattern):

```cpp
// CORRECT — check before computing
if (frame_count > SIZE_MAX / MAX_FRAME_PAYLOAD) {
  drop;  // or bound-check against max_frames_per_message_
}
size_t total = frame_count * MAX_FRAME_PAYLOAD;
```
...
### ❌ Manual `new` / `delete` (Raw Owning Pointers)

```cpp
// WRONG — manual ownership, leak risk on early return
auto *buf = new uint8_t[64];
if (!ok) { delete[] buf; return; }  // easy to forget
...
### ❌ Non-Trivially-Copyable Types Through FreeRTOS Queues

`xQueueSend`/`xQueueReceive` copy raw bytes (memcpy) — C++ constructors/destructors are NOT run.

```cpp
// WRONG — std::string memcpy'd through the queue; destructor never fires per item
std::string frame;
while (xQueueReceive(q, &frame, 0) == pdTRUE) { ... }

// CORRECT — POD/trivially-copyable structs, or pointers; drain with manual free + nullify
UploadJob job;
while (xQueueReceive(q, &job, 0) == pdTRUE) {
  this->process(job.data);
  free(job.data);      // manual free between iterations
  job.data = nullptr;  // destructor fires only for the LAST item
}
```

### ❌ `std::string` Temporaries in the Hot Path

```cpp
// WRONG — heap allocation per call
std::string tmp = std::to_string(value) + "°C";

// CORRECT — SSO-safe, pre-reserved scratch buffer (reserved in setup())
this->scratch_.clear();
this->scratch_.reserve(16);
...append into scratch_...
```
...
### 14.10 C++20 Is Expected

The toolchain targets gnu++20. Do NOT flag C++20 features (`std::array`, lambdas, `std::make_unique`, structured bindings, ...) as "too modern" or "incompatible with the platform" — but match what this codebase actually uses (§5.1).

### 14.11 Lessons Learned (C++ / FreeRTOS Impedance)

- **Tasks MUST exit their loop naturally** to unwind the stack (destructors fire on scope exit). Call `vTaskDelete(nullptr)` only at the very end of the task function.
- **NEVER send/receive non-TriviallyCopyable types via `xQueue`** — raw memcpy bypasses C++ assignment/destructors.
- **Structs drained with `xQueueReceive`:** when the receive variable lives outside the loop, manually `free()` + nullify between iterations — the destructor fires only for the LAST item received.
- **Dead-code hygiene:** when deleting a source file, remove ALL `#include` references to it across the whole tree.
- **Hot path:** prefer pre-allocated scratch buffers (reserved in `setup()`) over per-frame allocations, and integer arithmetic over `std::to_string()` + `strtol()` round-trips.
...
| **Field visibility** | `protected` default; `private` only for safety-critical (pointer lifetime, invariants, resources) | None |
| **Ownership** | `std::unique_ptr`/containers own buffers | Never raw owning pointers |
| **Thread safety** | `mutex` + `lock_guard` (or `std::atomic`) on callback <-> `loop()` shared state | None |
